...
def _upload_future_order(code, vol, orderid, straid, method="Normal"):
...
def _upload_opt_order(code, vol, orderid, straid, method="Normal"):
    SQL = "INSERT INTO TRADE_ORDER VALUES ({0},{1},'{2}',{3},'{4}','{5}','{6}')".format(
        straid, orderid, code, vol, method, str(datetime.datetime.now())[:19], 'init'
    )
    cur = Trading_CONN.cursor()
    cur.execute(SQL)
...
def send_order(code, price, vol, is_open, orderID, straID, **kwargs):
    ...
    if "method" in kwargs:
        ...
        order["method"] = "Normal"
    _upload_opt_order(code, vol, orderID, straID, order["method"])
    js = json.dumps(order)
...
def send_future_order(code, price, vol, is_open, orderID, straID, **kwargs):
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.connect(('10.0.0.111', 9987))
    order = {}
    order["code"] = code
    order["price"] = price
    order["vol"] = vol
    ...
    if "method" in kwargs:
        ...
        order["method"] = "Normal"
    _upload_future_order(code, vol, orderID, straID, order["method"])
    js = json.dumps(order)
    s.send(bytes(js, 'UTF-8'))
    s.close()
    return
